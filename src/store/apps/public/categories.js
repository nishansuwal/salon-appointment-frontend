...
export const fetchMainCategories = createAsyncThunk(
  `${modelName}s/fetchMainCategories`,
  async (params = {}, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get("/category-level/main", {
        params,
      });
...
export const fetchChildCategories = createAsyncThunk(
  `${modelName}s/fetchChildCategories`,
  async (params = {}, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get("/category-level/child", {
        params,
      });
...
const extraReducers = (builder) => {
  ...
  builder.addCase(fetchMainCategories.rejected, (state, action) => {
    state.message =
      action.payload?.message || "Failed to fetch main categories!";

    state.mainCategoriesLoading = false;
  });

  // CHILD CATEGORIES
  builder.addCase(fetchChildCategories.pending, (state) => {
    state.childCategoriesLoading = true;
  });

  builder.addCase(fetchChildCategories.fulfilled, (state, action) => {
    state.childCategories = action.payload.data ?? [];
    state.childCategoriesLoading = false;
  });

  builder.addCase(fetchChildCategories.rejected, (state, action) => {
    state.message =
      action.payload?.message || "Failed to fetch child categories!";
